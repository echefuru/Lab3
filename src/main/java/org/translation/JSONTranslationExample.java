...
public class JSONTranslationExample {
    ...
    public String getCountryNameTranslation(String countryCode, String languageCode) {
        for (int conNum = 0; conNum < jsonArray.length(); conNum++) {
            if (jsonArray.getJSONObject(conNum).getString("alpha3").equals(countryCode)) {
                return jsonArray.getJSONObject(conNum).getString(languageCode);
            }
        }
